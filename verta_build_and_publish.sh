...
if [ -n "$EXTRA_MAVEN_PARAMS" ]
then
  echo "using extra maven params $EXTRA_MAVEN_PARAMS"
fi

export MAVEN_PARAMS="-Pdist-hadoop3,hadoop3,bundle-contrib-exts -Dpmd.skip=true -Denforcer.skip -Dforbiddenapis.skip=true -Dcheckstyle.skip=true -Danimal.sniffer.skip=true -Djacoco.skip=true -DskipTests $EXTRA_MAVEN_PARAMS"
